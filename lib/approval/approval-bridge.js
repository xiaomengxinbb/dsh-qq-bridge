...
export class ApprovalBridge {
    ...
    buildKeyboard(approvalId) {
        const btn = (id, label, visited, choice, style) => ({
            id,
    ...
    buildText(toolName, reason, cwd) {
        const lines = ["🔐 **工具审批请求**", ""];
        if (toolName)
            lines.push(`🛠️ 工具: \`${toolName}\``);
        if (cwd)
            lines.push(`📁 目录: \`${cwd}\``);
        if (reason)
            lines.push(`📋 原因: ${reason.slice(0, 300)}`);
        lines.push("");
        lines.push("请在下方选择,或回复 `/approve`(允许一次)/`/always`(始终)/`/deny`(拒绝)");
        return lines.join("\n");
    }
    /** 解析按钮 data → 命中挂起审批则 resolve(返回 true=已处理) */
    handleButtonData(buttonData, operatorOpenId) {
        if (!buttonData.startsWith(APPROVAL_BUTTON_PREFIX))
            return false;
        ...
        const choice = choiceRaw === "once" || choiceRaw === "always" || choiceRaw === "deny"
            ? choiceRaw
            : "deny";
        return this.resolve(approvalId, operatorOpenId, choice);
    }
    /** 文本命令 /approve /always /deny → resolve */
    handleTextCommand(command, operatorOpenId) {
        const p = this.findForUser(operatorOpenId);
        if (!p)
            return { handled: false };
        let choice;
        if (command === "approve" || command === "yes" || command === "ok")
            choice = "once";
        else if (command === "always" || command === "allow-always")
            choice = "always";
        else if (command === "deny" || command === "no" || command === "cancel")
            choice = "deny";
        if (!choice)
            return { handled: false };
    ...
    /** 找该用户最早的挂起审批 */
    findForUser(userOpenId) {
        let oldest;
        for (const p of this.pending.values()) {
            if (p.userOpenId !== userOpenId)
                continue;
            if (!oldest || p.createdAt < oldest.createdAt)
                oldest = p;
        }
        return oldest;
    }
    /** 核心:resolve 审批(校验操作者身份) */
    resolve(approvalId, operatorOpenId, choice) {
        const p = this.pending.get(approvalId);
        if (!p)
            return false;
        // 安全:只有挂起审批的发起人能决定(仿 Hermes _is_authorized_interaction_for_session)
        if (p.userOpenId !== operatorOpenId)
            return false;
        clearTimeout(p.timer);
        this.pending.delete(approvalId);
        p.resolve(choice);
