...
function createBridge(cfg, ctx) {
    ...
    gateway.onInteraction((interaction) => {
        ...
        if (approvalBridge.handleButtonData(interaction.buttonData, interaction.operatorOpenId)) {
            logger?.info?.(`[dsh-qq-bridge] 审批按钮已处理: ${interaction.buttonData.slice(0, 60)} (by ${interaction.operatorOpenId.slice(0, 8)}...)`);
        }
    });
    const rt = {
        auth,
    ...
    if (ctx.on !== undefined && typeof ctx.on === "function") {
        ctx.on("approval/request", (req, next) => {
            ...
            try {
                ...
                const userOpenId = typeof sessionId === "string"
                ...
                return new Promise((resolveOutcome) => {
                    ...
                    void rt.api
                        .sendMarkdown({ type: "private", userOpenId, msgId }, text, Date.now() % 65535, keyboard)
                        .catch((err) => {
