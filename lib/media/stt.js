...
export async function transcribeOpenAI(input, config, outerSignal) {
    const key = process.env[config.apiKeyEnv ?? "QQBOT_STT_API_KEY"];
    if (!key)
        throw new SttError("stt_key_missing", `未设置 STT 密钥环境变量 ${config.apiKeyEnv ?? "QQBOT_STT_API_KEY"}`);
    if (!config.baseUrl)
    ...
    try {
        ...
        try {
            ...
            if (outerSignal.aborted)
                throw new SttError("aborted", "语音处理已取消");
