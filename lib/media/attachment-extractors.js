...
export async function extractTxt(path, maxChars) {
...
export async function extractPdf(path, maxPages, maxChars) {
    const bytes = await readFile(path);
    let pdf;
    try {
        ...
        if (!text.trim())
            throw new AttachmentExtractError("pdf_no_text", "PDF 没有可提取的文本层；当前版本不支持 OCR");
        return { ...truncateText(text, maxChars), pages: result.totalPages };
    }
    catch (err) {
        if (err instanceof AttachmentExtractError)
            throw err;
...
export function makeMinimalPdf(text) {
    const escaped = text
        .replace(/\\/g, "\\\\")
        .replace(/\(/g, "\\(")
        .replace(/\)/g, "\\)");
...
//# sourceMappingURL=attachment-extractors.js.map