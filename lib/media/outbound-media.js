...
import { QQApiError } from "../gateway/qq-api.js";
...
export class QQOutboundDeliveryContext {
    ...
    async sendLocalFile(inputPath, requestedKind = "auto") {
        let opened;
        let filename = "file";
        let bytes = 0;
        let mediaKind = requestedKind === "image" ? "image" : "file";
        ...
        try {
            ...
            if (pinnedPath !== path)
                throw new QQOutboundMediaError("file_changed", "文件路径在打开过程中发生变化，请重试");
            const before = await opened.stat();
            if (!before.isFile())
                throw new QQOutboundMediaError("not_regular_file", "目标不是普通文件");
            if (before.nlink > 1)
                throw new QQOutboundMediaError("hardlink_not_allowed", "为避免越权读取，不允许发送硬链接文件");
            bytes = before.size;
            if (bytes <= 0)
            ...
            this.assertAvailable();
            phase = "upload";
            const uploaded = await this.options.api.uploadMedia(this.options.target, mediaKind === "image" ? 1 : 4, fileData, this.options.signal, this.options.config.outboundMedia.uploadTimeoutMs);
            const msgSeq = this.options.reserveMessageSequence();
            if (msgSeq === undefined)
                throw new QQOutboundMediaError("reply_budget_exhausted", "QQ 被动回复配额不足，已取消媒体发送");
            phase = "send";
            try {
    ...
    assertAvailable() {
        ...
        if (this.options.signal?.aborted)
            throw new QQOutboundMediaError("delivery_context_closed", "当前 QQ 回合已经被停止");
    }
    assertAuthorized() {
        const policy = this.options.config.outboundMedia;
        if (!policy.enabled)
            throw new QQOutboundMediaError("outbound_disabled", "电脑文件发送功能尚未启用");
        ...
        if (policy.adminsOnly &&
            !this.options.config.commands.admins.includes(this.options.message.userOpenId)) {
            throw new QQOutboundMediaError("outbound_not_authorized", "只有显式配置的 QQ 管理员可以发送电脑文件");
        }
    }
    failureRecord(filename, kind, bytes, errorCode, note, status = "failed") {
...
export async function resolveAllowedLocalFile(input, cwd, configuredRoots) {
...
export function normalizeInputPath(input, cwd) {
...
function normalizeQQError(err, phase) {
...
//# sourceMappingURL=outbound-media.js.map