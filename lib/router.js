 ...
 */
import { MessageDedupe } from "./session/message-dedupe.js";
import { ReplyBudget } from "./session/reply-budget.js";
import { formatUserFacingAgentError } from "./core/user-facing.js";
import { CommandStateMachine, authorizeQQCommand, } from "./commands/command-controller.js";
import { parseQQCommand } from "./commands/command-parser.js";
import { buildModelPage, formatModelPageFallback, } from "./commands/model-pages.js";
import { buildCommandKeyboard } from "./commands/qq-keyboard.js";
import { formatAttachmentFailures, hasUsableAgentInput, } from "./media/attachment-pipeline.js";
import { QQOutboundDeliveryContext } from "./media/outbound-media.js";
import { formatQQReply } from "./reply-formatter.js";
...
export class QQRouter {
    ...
    lastMsgIdByUser = new Map();
    constructor(config, registry, api, options = {}) {
        this.config = config;
     ...
     */
    startTypingIndicator(msg) {
        if (msg.type !== "private")
            return undefined; // C2C 专用
        const msgId = msg.id;
        if (!msgId)
            return undefined;
    ...
    async executeCommand(msg, command) {
        switch (command.name) {
            case "help":
            ...
            case "workspace":
                ...
                return;
            default:
                await this.replyToQQ(msg, `## 未知命令\n\n\`/${command.name}\` 不在支持列表中。发送 \`/help\` 查看。`);
        }
    }
    async getConversation(msg) {
        return this.registry.get(msg);
    }
    async handleModelCommand(msg, rawArgs) {
        ...
        if (!query) {
            const modelPage = buildModelPage(all, page, this.config.commands.modelPageSize);
            const lines = [
                "## 当前 QQ 模型",
                "",
        ...
        // 精确匹配 → 直接切换
        const exact = all.find((model) => `${model.provider}/${model.id}`.toLowerCase() === query);
        if (exact) {
    ...
    async handleNewCommand(msg, name) {
        ...
        await this.replyToQQ(msg, `## 已新建 QQ 会话\n\n- 会话：${created.name ? `**${created.name}**` : "未命名"}\n- ID：\`${created.id.slice(0, 8)}\`\n- 模型：\`${model ? `${model.provider}/${model.id}` : "unknown"}\`\n\n直接发送新任务即可；旧会话仍保存在历史中。`, this.helpKeyboard(msg));
    }
    async handleSessionsCommand(msg, rawArgs) {
        const qq = await this.getConversation(msg);
        const all = await qq.listSessions();
        const query = rawArgs.trim().toLowerCase();
        const sessions = (query ? all.filter((s) => sessionMatches(s, query)) : all).slice(0, this.config.commands.maxListItems);
        if (!sessions.length) {
    ...
    async handleResumeCommand(msg, selector) {
        const key = conversationKeyOf(msg);
        const pending = this.stateMachine.get(key);
        if (pending?.command === "resume" && selector && /^\d+$/.test(selector)) {
            ...
            if (!target)
                throw new Error("序号无效；请重新发送 /resume");
            this.stateMachine.clear(key);
        ...
        const matches = all.filter((s) => s.id.toLowerCase().includes(normalized) ||
            (s.name ?? "").toLowerCase().includes(normalized));
        if (!matches.length)
            throw new Error(`没有找到匹配“${selector}”的会话；发送 /sessions 查看历史`);
        if (matches.length === 1) {
    ...
    async handleStopCommand(msg) {
        ...
        await this.replyToQQ(msg, aborted || removed > 0
            ? `## 已停止 QQ 任务\n\n${aborted ? "当前生成已中止。" : ""}${removed > 0 ? ` 已移除 ${removed} 条待处理消息。` : ""}\n\nQQ 会话历史已保留。`
            : "当前 QQ 会话没有正在执行或等待的任务。");
    }
    async handleWorkspaceCommand(msg, args) {
        ...
        if (args.length === 0) {
            ...
            const lines = [
                ...
                ...registry
                    .list()
                    .map((w) => `- \`${w.name}\`  ${w.path}${w.description ? `（${w.description}）` : ""}`),
                "",
        ...
        await this.replyToQQ(msg, `## 已切换工作区\n\n- 工作区：**${resolved.name}**\n- 路径：\`${resolved.path}\`\n- 会话已重置到该目录，直接发送任务即可。`, this.helpKeyboard(msg));
    }
    // ── 回复文本辅助 ───────────────────────────────────────────────
    commandHelp(target) {
        const help = {
        ...
        const lines = [
            ...
            "",
            "普通文本会作为任务发给 DeepSeek Harness Agent。管理命令需要管理员权限。",
        ];
        return lines.join("\n");
    }
    async statusText(msg) {
        const qq = await this.getConversation(msg);
        const model = qq.currentModel();
        return [
            "## QQ 会话状态",
    ...
    lastSummary() {
        const inbound = this.recentInbound
            .slice(-5)
            .map((e) => `- ${new Date(e.at).toLocaleTimeString()} 入站：${truncate(e.text, 40)}`);
        const outbound = this.recentOutbound
            .slice(-5)
            .map((e) => `- ${new Date(e.at).toLocaleTimeString()} 出站：${truncate(e.text, 40)}`);
        const lines = ["## 最近活动", ""];
        ...
        if (inbound.length)
            lines.push("### 入站", ...inbound, "");
        if (outbound.length)
            lines.push("### 出站", ...outbound);
        return lines.join("\n");
    }
    // ── 键盘 ───────────────────────────────────────────────────────
    helpKeyboard(msg) {
        if (!this.config.commands.buttons)
            return undefined;
        const rows = [
            [
    ...
    async steerInto(active, msg) {
        try {
            ...
            if (this.activeConversation?.key === active.key &&
    ...
    async runOne(msg) {
        ...
        const ackTimer = this.config.progress.enabled && this.config.progress.ackAfterMs > 0
            ? setTimeout(() => {
        ...
        try {
            ...
            if (preparedCleanup)
                await preparedCleanup().catch(() => undefined);
        }
    }
    /** 发送回复（命令/拒绝/申请共用）：分块发送，每块占 1 次配额 */
    async replyToQQ(msg, content, keyboard) {
        await this.sendFormatted(msg, content, keyboard, true);
    }
    /** 发送日志辅助 */
    debugSend(tag, detail) {
        this.debugLog?.(`[router] ${tag} ${detail}`);
    }
    /** 分块 + Markdown 优先（降级纯文本保持 msg_seq 对齐） */
    async sendFormatted(msg, content, keyboard, fallbackToPlain, budget) {
    ...
    recordInbound(msg) {
        ...
        if (this.recentInbound.length > LAST_RING_MAX)
            this.recentInbound.shift();
    ...
    lastMsgIdFor(userOpenId) {
        return this.lastMsgIdByUser.get(userOpenId);
    }
    recordOutbound(_msg, text, _seq) {
        this.recentOutbound.push({ at: Date.now(), text });
        if (this.recentOutbound.length > LAST_RING_MAX)
