...
export function formatQQReply(text, mode) {
    const normalized = normalizeMarkdown(text);
    const source = truncateUtf8(normalized, MAX_SOURCE_BYTES);
    const markdownChunks = chunkMarkdown(source.text, QQ_MARKDOWN_CHUNK_BYTES - PART_LABEL_RESERVE_BYTES, QQ_MAX_REPLY_CHUNKS);
...
export function chunkMarkdown(text, maxBytes, maxChunks) {
...
export function chunkPlainText(text, maxBytes, maxChunks) {
    if (utf8Bytes(text) <= maxBytes)
        return [text];
    ...
    if (chunks.reduce((total, chunk) => total + utf8Bytes(chunk), 0) <
        utf8Bytes(text) &&
        chunks.length) {
        chunks[chunks.length - 1] = appendWithinBudget(chunks[chunks.length - 1], "\n\n⚠️ 回复过长，后续内容已省略。", maxBytes);
    }
            ...
            if (headers.length >= 2 && rows.length) {
                for (const row of rows) {
                    ...
                    for (let column = 1; column < headers.length; column++) {
                        ...
                        if (value)
                            output.push(`  - **${escapeMarkdownLabel(headers[column])}：**${value}`);
                    }
                }
                continue;
            }
...
function attachHeadings(blocks) {
    const result = [];
...
function splitFencedBlock(block, maxBytes) {
        ...
        if (utf8Bytes(candidate) + utf8Bytes(closing) <= maxBytes) {
            ...
            if (utf8Bytes(line) >
                ...
                for (const chunk of chunks) {
                    if (pieces.length >= QQ_MAX_REPLY_CHUNKS)
                        break;
                    pieces.push(`${opening}\n${chunk}\n${closing}`);
                }
                current = "";
    ...
    if (current && currentBytes > 0)
        pieces.push(`${current}\n${closing}`);
    return pieces;
}
function splitSemantic(text, maxBytes) {
    const lines = text.split("\n");
    const pieces = [];
    let current = "";
    for (const line of lines) {
        ...
        if (utf8Bytes(line) > maxBytes) {
            for (const chunk of splitLine(line, maxBytes))
                pieces.push(chunk);
        }
        else
            current = line;
...
function splitLine(line, maxBytes) {
    const chunks = [];
    let current = "";
...
function withPartLabels(chunks, markdown) {
    if (chunks.length <= 1)
        return chunks;
...
function appendWithinBudget(chunk, suffix, maxBytes) {
