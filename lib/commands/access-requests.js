    ...
    createCode() {
        for (;;) {
            const code = randomBytes(5)
                ...
                .replace(/[-_]/g, "")
                .slice(0, 6);
            if (code.length === 6 && !this.byCode.has(code))
                return code;
        }
    }
}
...
/** 只保留消息元数据，正文与附件一律清空 */
function redactRequestMessage(message) {
    return {
        id: message.id,
        type: "private",
        text: "",
