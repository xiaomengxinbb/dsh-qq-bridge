...
function dbgStart(...args: unknown[]): void {
  if (process.env.QQBOT_DEBUG_START !== "1") return;
  try {
...
function loadConfigOnce(): PiQQBridgeConfig | undefined {
  ...
  try {
    ...
    state.configError =
      err instanceof ConfigError ? err.message : `配置加载失败：${(err as Error).message}`;
    state.config = undefined;
  }
  return state.config;
}

function debugLogFor(cfg: PiQQBridgeConfig): ((message: string) => void) | undefined {
  if (!cfg.debug) return undefined;
  return (message: string) => {
    try {
      appendFileSync(
        "/tmp/dsh-qq-bridge-gw.log",
...
function logRouterEvent(event: QQRouterEvent): void {
...
function createBridge(cfg: PiQQBridgeConfig, ctx: any): BridgeRuntime {
  const logger = state.logger;
  ...
  const registry = new ConversationRegistry(
    ...
    agentDir,
    cwd,
    { create: () => new QQAgentSession(host) },
...
async function startBridge(cfg: PiQQBridgeConfig, ctx: any): Promise<string> {
  ...
  if (!acquired.held) return `无法启动：${acquired.reason}`;
...
function applyApproval(cfg: PiQQBridgeConfig, userOpenId: string, role: "user" | "admin"): void {
  const updated = {
    ...cfg,
...
export function apply(ctx: any, _config: unknown): void {
  ...
  if (!cfg) {
    ...
    logger.info?.(
      `[dsh-qq-bridge] 配置已装载（schemaVersion ${cfg.schemaVersion}，sandbox=${cfg.sandbox}，mode=${cfg.startup.mode}）`,
    );
  }
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      ...
      if (!cfgNow) return { kind: "error" as const, text: `配置不可用：${state.configError ?? "未加载"}` };
      return { kind: "success" as const, text: await startBridge(cfgNow, ctx) };
    },
  });
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      ...
      if (!rt) return { kind: "success" as const, text: "QQ 网关未运行" };
      ...
      if (rt.lockCheckTimer) clearInterval(rt.lockCheckTimer);
      state.runtime = undefined;
      return { kind: "success" as const, text: "QQ 网关已停止" };
    },
  });
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      const rt = state.runtime;
      if (!rt) return { kind: "error" as const, text: "QQ 网关未运行（先 /qqbot-start）" };
      const ok = await rt.gateway.reconnect();
      return {
        kind: ok ? ("success" as const) : ("error" as const),
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      ...
      if (!requests.length) return { kind: "success" as const, text: "没有待审批的访问申请" };
      const lines = [
        "## 待审批访问申请",
        "",
        ...requests.map((r) =>
  ...
  ctx.commands.register({
    ...
    handler: async (_invocation: { rawInput?: string }) => {
      const rt = state.runtime;
      const cfgNow = loadConfigOnce();
      if (!rt || !cfgNow) return { kind: "error" as const, text: "QQ 网关未运行或配置不可用" };
      const raw = (_invocation as { rawInput?: string }).rawInput ?? "";
      ...
      if (!code || !role) {
        return { kind: "error" as const, text: "用法：/qqbot-approve <申请码> <user|admin> [--yes]" };
      }
      const request = rt.accessRequests.approve(code);
      if (!request) return { kind: "error" as const, text: `申请码 ${code} 不存在或已过期` };
      if (role === "admin" && !force) {
      ...
      return { kind: "success" as const, text: `已批准 ${request.userOpenId}（${role}）` };
    },
  });
  ...
  ctx.commands.register({
    ...
    handler: async (_invocation: { rawInput?: string }) => {
      ...
      if (!code) return { kind: "error" as const, text: "用法：/qqbot-deny <申请码>" };
      const request = rt.accessRequests.deny(code);
      if (!request) return { kind: "error" as const, text: `申请码 ${code} 不存在或已过期` };
      return { kind: "success" as const, text: `已拒绝 ${request.userOpenId}（1 小时内不再接收其申请）` };
    },
  });
  ...
  ctx.commands.register({
    ...
    handler: async (_invocation: { rawInput?: string }) => {
      const cfgNow = loadConfigOnce();
      if (!cfgNow) return { kind: "error" as const, text: "配置不可用" };
      const raw = (_invocation as { rawInput?: string }).rawInput ?? "";
      const tokens = raw.trim().split(/\s+/).filter(Boolean);
      const openid = tokens[0];
      const force = tokens.includes("--yes");
      if (!openid) return { kind: "error" as const, text: "用法：/qqbot-revoke <user_openid> [--yes]" };
      if (!force) {
        return {
          kind: "success" as const,
      ...
      return { kind: "success" as const, text: `已撤销 ${openid} 的权限` };
    },
  });
  ...
  ctx.commands.register({
    ...
    handler: async (_invocation: { rawInput?: string }) => {
      const rt = state.runtime;
      const cfgNow = loadConfigOnce();
      if (!rt || !cfgNow) return { kind: "error" as const, text: "QQ 网关未运行或配置不可用（先 /qqbot-start）" };
      const raw = (_invocation as { rawInput?: string }).rawInput ?? "";
      const tokens = raw.trim().split(/\s+/).filter(Boolean);
      ...
      if (tokens[0] === "add") {
        ...
        if (!wName || !wPath) return { kind: "error" as const, text: "用法：/workspace add <名称> <绝对路径> [描述]" };
        try {
          ...
          return { kind: "success" as const, text: `已添加工作区 ${workspace.name} → ${workspace.path}` };
        } catch (err) {
          return { kind: "error" as const, text: `${(err as Error).message}` };
        }
      }
      if (tokens[0] === "remove") {
        const wName = tokens[1];
        if (!wName) return { kind: "error" as const, text: "用法：/workspace remove <名称>" };
        try {
          registry.remove(wName);
          cfgNow.workspaces = registry.list().filter((w) => w.name !== "default");
          saveConfig(expandHome(DEFAULT_CONFIG_PATH), cfgNow);
          return { kind: "success" as const, text: `已移除工作区 ${wName}` };
        } catch (err) {
          return { kind: "error" as const, text: `${(err as Error).message}` };
        }
      }
      try {
        ...
        if (rt.registry.currentWorkspace.name === resolved.name) {
          return { kind: "success" as const, text: `已在工作区 ${resolved.name}（${resolved.path}）` };
        }
        await rt.registry.setWorkspace(resolved.name, resolved.path);
        return { kind: "success" as const, text: `已切换工作区：${resolved.name}（${resolved.path}）` };
      } catch (err) {
        return { kind: "error" as const, text: `${(err as Error).message}` };
      }
    },
  ...
  if (process.env.DSH_QQBRIDGE_SELFTEST === "1") {
    void import("./dev/self-test.ts").then(({ runSelfTest }) => runSelfTest(ctx));
  }
}

/** 开发期冒烟标记：仅 DSH_QQBRIDGE_BOOT_MARKER=1 时写入（默认关闭） */
function writeBootMarker(logger: any): void {
  if (process.env.DSH_QQBRIDGE_BOOT_MARKER !== "1") return;
  try {
    mkdirSync(process.cwd(), { recursive: true });
    writeFileSync(
      process.cwd() + "/.boot-marker",
      JSON.stringify({ applied: true, at: Date.now(), pid: process.pid }, null, 2),
    );
  } catch (err) {
